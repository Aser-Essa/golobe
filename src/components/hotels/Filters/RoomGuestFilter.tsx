...
export default function RoomGuestFilter({
  ...
  const roomsCount = Number(roomsFormValue);
  const guestsCount = Number(guestsFormValue);

  const maxGuests = roomsCount * 3;

  const increaseRooms = () => {
    if (roomsCount >= 10) return;
    setValue("rooms", roomsCount + 1);
  };
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          data-empty={""}
          className="data-[empty=true]:text-muted-foreground relative h-14 w-[240px] flex-1! justify-between rounded-[4px] border-[#79747E] bg-white! px-4 text-left font-normal"
        >
          <div className="flex items-center justify-start gap-2">
            <UserRound className="size-5" />
            <span>
              {roomsCount} room{roomsCount > 1 && "s"} - {guestsCount} guest
              {guestsCount > 1 && "s"}
            </span>
          </div>
        ...
        <div className="w-full">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <p className="font-semibold">{roomsCount}</p>
              <Button
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <p className="font-semibold">{guestsCount}</p>
              <Button
